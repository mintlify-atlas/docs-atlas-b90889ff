---
title: "DTOs and Pagination in FoundationKit"
sidebarTitle: "DTOs & Pagination"
description: "Learn how FoundationKit's BaseInput, BaseEdit, BaseOutput, Paginate, and PaginationResult types standardize data transfer and list responses across your application."
---
...
<ParamField query="NoPaginate" type="bool" default="false">
  When `true`, pagination is skipped entirely and all matching records are returned in a single `PaginationResult` with `ActualPage`, `Qyt`, `PageTotal`, and `Total` all set to `0` or `1`. Useful for dropdown/autocomplete scenarios where you need the full list.
</ParamField>
...
<ResponseField name="pageTotal" type="int">
  The total number of pages available, calculated as `ceil(Total / Qyt)`. Useful for rendering pagination controls on the client.
</ResponseField>

<ResponseField name="total" type="int">
  The total number of records that match the filter, across all pages. `0` when `NoPaginate` is `true`.
</ResponseField>

<ResponseField name="query" type="string">
  Echo of the search term from the `Paginate` request, or `null` if none was supplied. Useful for the client to display "showing results for…" messages.
</ResponseField>

<ResponseField name="results" type="IReadOnlyCollection<T>">
  The records for the current page. An empty collection (never `null` in practice) when no records match.
</ResponseField>
...
Asynchronously materialises the query into a `PaginationResult<T>`. Respects `NoPaginate` — when set, all results are returned without slicing.
