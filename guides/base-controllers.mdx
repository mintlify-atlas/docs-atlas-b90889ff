---
title: "Abstract Base Controllers for API and MVC Apps"
...
<Note>
All three controllers use the `[Route("api/[controller]")]` attribute convention. The controller name is derived from the class name with the `Controller` suffix removed. For MVC controllers this works alongside the default `{controller}/{action}` convention.
</Note>
