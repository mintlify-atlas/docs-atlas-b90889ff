...
<CardGroup cols={2}>
  ...
  <Card title="FoundationKit.EntityFrameworkCore" icon="database">
    Published as **FoundationKit.EntityFrameworkCore** on NuGet (the solution project is named `FoundationKit.Domain`). Provides `BaseModel`, `FoundationKitDbContext`, `FoundationKitIdentityDbContext`, and EF Core configuration helpers backed by `Microsoft.EntityFrameworkCore 8.0.4`.
  </Card>
  <Card title="FoundationKit.Repository" icon="arrows-split-up-and-left">
    Provides `IBaseRepository<TModel>`, `BaseRepository<TContext, TModel>`, `IMapRepository`, `MapRepository`, and the `PaginationResult<T>` / `Paginate` value objects. Depends only on `FoundationKit.EntityFrameworkCore` and `AutoMapper 13.0.1`.
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card title=".NET 8 & NuGet Distribution" icon="cube">
    All packages target `net8.0` exclusively and are published to NuGet.org under version **4.1.0**. CI publishing is handled by the `Release to NuGet` GitHub Actions workflow.
  </Card>
</CardGroup>

## Target Framework & Distribution

All four packages target **`net8.0`** and are distributed via NuGet.org. The current stable release is **4.1.0**. Source code is available at [github.com/Orbis25/FoundationKit](https://github.com/Orbis25/FoundationKit) under the MIT licence.
