---
title: "IMapRepository<TInput, TEdit, TDto> — AutoMapper Repository Interface"
sidebarTitle: "IMapRepository"
description: "IMapRepository defines CRUD operations that map between entity, input, edit, and output DTO types using AutoMapper — the core contract for MapRepository."
---
...
Maps a `TEditModel` to the underlying entity and persists the update. When `verifyEntity` is `true`, the repository fetches the current DTO first to preserve `CreatedBy` and `CreatedAt` audit fields.
...
<ParamField path="verifyEntity" type="bool" default="true">
  When `true`, performs a `GetById` before updating to copy immutable audit fields (`CreatedBy`, `CreatedAt`) from the existing record onto the edit model. Set to `false` to skip the pre-fetch if you have pre-populated those fields yourself.
</ParamField>
